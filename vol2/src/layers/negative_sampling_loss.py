...
class NegativeSamplingLoss:
    def __init__(self, W, corpus, sample_size=5, power=0.75):
        ...
        self.embedding_dot_layers = [
            EmbeddingDot(W) for _ in range(
                sample_size + 1)]
        self.params = []
        self.grads = []
    ...
    def forward(self, h, target):
        ...
        for i in range(self.sample_size):
            ...
            score = self.embedding_dot_layers[1 +
    ...
    def backward(self, dout=1):
        dh = 0
        for loss_layer, embedding_dot_layer in zip(
                self.loss_layers, self.embedding_dot_layers):
