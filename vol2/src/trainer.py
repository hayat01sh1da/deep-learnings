...
class Trainer:
            ...
            for i in range(0, L - 1):
                for j in range(i + 1, L):
                    ...
                    if params[i] is params[j]:
                        ...
                        params.pop(j)
                        grads.pop(j)
                    # In case of sharing heaviness as transpose list(weight
                    # typing)
    ...
    def _evaluate(
        ...
        training_status = '| epoch %d |  iter %d / %d | time %d[s] | loss %.2f' % (
            current_epoch + 1, iters + 1, max_iters, elapsed_time, average_loss)
        ...
        for epoch in range(max_epoch):
            ...
            for iters in range(max_iters):
                ...
                if (eval_interval is not None) and (
                        iters % eval_interval) == 0:
                    average_loss, training_status = self._evaluate(
                        total_loss, loss_count, start_time, current_epoch, iters, max_iters)
                    total_loss = 0
                    loss_count = 0
