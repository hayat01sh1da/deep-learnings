...
class TrainCustomLoop:
    ...
    def _learning_process(
        ...
        process = '| epoch %d | iter %d / %d | loss %.2f' % (
            epoch + 1, iters + 1, max_iters, average_loss)
    ...
    def update(self, x, t, max_epoch, batch_size):
        ...
        for epoch in range(max_epoch):
            ...
            for iters in range(max_iters):
                ...
                loss = self._update_params_with_grads(
                    batch_x, batch_t, total_loss, loss_count)
                total_loss += loss
                loss_count += 1
                if (iters + 1) % 10 == 0:
                    average_loss, * \
                        _ = self._learning_process(total_loss, loss_count, epoch, iters, max_iters)
