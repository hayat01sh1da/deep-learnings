...
@pytest.fixture
def lstm_setup():
    Wx = np.array([[9.72009451e-01, -
                    4.97642862e-01, 6.45448952e-01, 8.10387855e-01, 1.13757673e+00, -
                    5.27114694e-01, -
                    9.08624540e-01, 1.61896844e+00, -
                    1.16690977e+00, 3.93476226e-01, -
                    6.04018422e-01, 5.67830817e-01], [6.68131790e-01, 6.40157016e-01, 6.90200961e-01, -
                                                      1.39750585e+00, -
                                                      4.89624070e-01, 8.99789953e-01, 3.97067428e-04, 1.47459503e+00, -
                                                      4.95030269e-01, -
                                                      9.22541855e-01, -
                                                      1.57352198e-01, -
                                                      1.67160494e+00], [6.93508859e-01, -
                                                                        9.23177216e-01, -
                                                                        4.83511551e-01, -
                                                                        1.18675890e+00, -
                                                                        7.35505045e-01, -
                                                                        1.61403611e+00, -
                                                                        2.76067694e-02, -
                                                                        2.48294747e-01, 1.14474446e+00, 1.86354309e-01, -
                                                                        1.73018002e+00, -
                                                                        4.82520536e-03], ])
    Wh = np.array([
        [-0.88480318, 1.09509583, 0.55657863, -0.35096014, 0.18572107, 0.83823659, -0.44063768, -0.80897913, 0.35752315, 1.65812611, 1.40425671, 1.52519905],
        [-0.22279229, 1.16363656, -0.47632291, -0.16436909, -2.16120359, 0.28362134, 0.01817155, 0.04836914, -0.30831619, -2.0992645, -0.07302497, -0.72868125],
        [-1.40551611, 2.12755955, 1.76232202, 2.15703084, -1.87387492, 1.22755896, -0.84271588, 1.07860737, -0.35473314, -0.86293879, 1.67287773, 0.41575087],
    ])
    b = np.array([
        0.46861655, 0.15954682, 0.38782221, 1.00791178, -0.38322573, 0.83138721,
        0.98675017, -0.83388618, 1.14392808, 0.37846653, 0.47617248, -1.8035631,
    ])
    lstm = LSTM(Wx, Wh, b)
    x = np.array([
    ...
    return lstm, x, h_prev, c_prev


_EXPECTED_C_NEXT = np.array([
...
def test_forward(lstm_setup):
    lstm, x, h_prev, c_prev = lstm_setup
    c_next, h_next = lstm.forward(x, h_prev, c_prev)
    assert_almost_equal(c_next, _EXPECTED_C_NEXT)
    assert_almost_equal(h_next, _EXPECTED_H_NEXT)


def test_backward(lstm_setup):
    lstm, x, h_prev, c_prev = lstm_setup
    dc_next, dh_next = lstm.forward(x, h_prev, c_prev)
    dx, dh_prev, dc_prev = lstm.backward(dc_next, dh_next)
    assert_almost_equal(dx, _EXPECTED_DX)
    assert_almost_equal(dh_prev, _EXPECTED_DH_PREV)
    assert_almost_equal(dc_prev, _EXPECTED_DC_PREV)


def test_grads_diff(lstm_setup):
    lstm, x, h_prev, c_prev = lstm_setup
    before_Wx_grads, before_Wh_grads, before_b_grads = (
        copy.copy(g) for g in lstm.grads)

    dc_next, dh_next = lstm.forward(x, h_prev, c_prev)
    lstm.backward(dc_next, dh_next)

    after_Wx_grads, after_Wh_grads, after_b_grads = lstm.grads
    assert_array_equal(
        before_Wx_grads == after_Wx_grads, np.zeros(
            (3, 12), dtype=bool))
    assert_array_equal(
        before_Wh_grads == after_Wh_grads, np.zeros(
            (3, 12), dtype=bool))
    assert_array_equal(
        before_b_grads == after_b_grads,
        np.zeros(
            12,
            dtype=bool))
