import copy

import numpy as np
import pytest
from numpy.testing import assert_array_equal

from time_embedding import TimeEmbedding


@pytest.fixture
def setup():
    W = np.array([
        [0.3280114, 0.51719588, 2.56566087],
        [0.53182669, 0.85125353, -0.05985388],
        [-1.03053182, 0.89397269, 1.24217588],
        [-0.7373072, 0.57723267, -1.13001464],
        [-0.90229024, 0.97809838, -0.09150674],
        [0.71714065, -1.28034957, 0.693378],
        [-2.13395059, 1.04338757, 1.14265363],
    ])
    return TimeEmbedding(W), np.array([[0, 4, 4, 1], [4, 0, 2, 1]])


def test_forward(setup):
    time_embedding, xs = setup
    assert time_embedding.forward(xs).shape == (2, 4, 3)


def test_grads_diff(setup):
    time_embedding, xs = setup
    before_grads = copy.copy(time_embedding.grads[0])

    dout = time_embedding.forward(xs)
    time_embedding.backward(dout)

    after_grads = time_embedding.grads[0]
    assert_array_equal(
        before_grads == after_grads,
        np.array([
            [False, False, False],
            [False, False, False],
            [False, False, False],
            [True, True, True],
            [False, False, False],
            [True, True, True],
            [True, True, True],
        ]),
    )
